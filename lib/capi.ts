...
export async function sendCAPIEvent({
  ...
  // externalId aumenta muito o match rate — usa email ou cpf como identificador único
  if (customer.externalId) userData.external_id = sha256(customer.externalId);
  // fbc e fbp NÃO são hasheados — enviados como plain text
  if (customer.fbc)        userData.fbc         = customer.fbc;
  if (customer.fbp)        userData.fbp         = customer.fbp;
  // IP e UA também plain text
